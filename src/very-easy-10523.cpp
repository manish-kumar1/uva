...
//x = 1*r + 2*r^2 + 3*r^3 + ... + n*r^n
//x/r = 1 + 2*r + 3*r^2 + .. + n*r^(n-1) = d(r + r^2 + r^3 + ... + r^n)/dr = d((1-r^(n+1))/(1-r)) = (-(n+1)r^n)/(1-r) - (1-r^(n+1)/(1-r)^2
//x = (n+1)*r^(n+1)/(r-1) - (1-r^(n+1))/(1-r)^2
//2*3^4 - (1-3^4)/4
...
int main() {
    ...
    while (!(cin >> n >> a).eof()) {
        if (a == 0) cout << a << endl;
        else if (a == 1) cout << (n*(n+1))/2 << endl;
        else {
            big_unsigned A(a);
            big_unsigned t = A^(n+1);
            unsigned a1 = a-1, na1 = n*a1, a12 = a1*a1;
            cout << ((t*(na1 - 1) + A)/a12) << endl;
            
//            cout << ((a + ((a^(n+2))*n - (n+1)*(a^(n+1))))/((a-1)^2)) << endl;
        }
//        cout << ((big_unsigned(n+1)*(a^n))/(1+a^2 - a*2)) << endl;
//        cout << (n/a) << endl;
    }
