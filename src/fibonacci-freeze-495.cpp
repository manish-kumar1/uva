...
int main() {
    unsigned long long tmp = 1, last = 1, secondLast = 1;
    vector<unsigned long long> t(94, 0ull);
    vector<big_unsigned> bigt;
    unsigned a, b, biggest = 93;

    t[1] = 1ull;
    t[2] = 1ull;

    for (unsigned long long i = 3; i < 94; ++i) {
        t[i] = t[i-1] + t[i-2];
    }

    big_unsigned big_tmp(t[93]), big_last(t[93]), big_secondLast(t[92]);

    bigt.push_back(big_secondLast);
    bigt.push_back(big_last);

    while (!(cin >> a).eof()) {
        cout << "The Fibonacci number for " << a << " is ";
        if (a < 94 ) cout << t[a] << endl;
        else if (a < biggest) {
            cout << bigt[a-92] << endl;
        } else {
            for (; biggest <= a; ++biggest) {
                bigt.push_back(bigt[biggest-92] + bigt[biggest-92-1]);
            }
