...
big_unsigned karatsuba(const big_unsigned& a, const big_unsigned& b) {
    ...
    if (M2 > 8192) {
        ...
        auto z0 = std::async(std::launch::async, karatsuba, low1, low2);
        auto z2 = std::async(std::launch::async, karatsuba, high1, high2);
        auto s1 = std::async(std::launch::async, operator+, low1, high1);
        auto s2 = std::async(std::launch::async, operator+, low2, high2);
        auto z1 = std::async(std::launch::async, karatsuba, s1.get(), s2.get());
        auto numZ2 = z2.get(), numZ0 = z0.get();
        big_unsigned t = numZ2 + numZ0;
        return numZ2.append(0, 2*M2) + ((z1.get() - t).append(0, M2)) + numZ0;
    }
    else {
        big_unsigned z0 = karatsuba(low1, low2);
        big_unsigned z2 = karatsuba(high1, high2);
...
int main() {
  vector<big_unsigned> bignum {big_unsigned(9223372036854775808ull)};
  vector<int> idx (1024, -1);
  idx[0] = 0;
  idx[1] = 0;
  idx[2] = 1;
  idx[3] = 2;
  ...
  uint64_t i = 1;
//  for(uint64_t n = 1; n < numeric_limits<uint64_t>::max(); ++i) {
//    n = n*i;
//    cout << i << ", " << n << endl;
//  }

  for (unsigned i = 66; i <= 1000; ++i) {
    bignum.emplace_back(big_unsigned(i)*bignum[i-66]);
  }

//  for_each(istream_iterator<unsigned>(cin), istream_iterator<unsigned>(), [&idx,&bignum](const unsigned& n) {
//    if (idx[n] < 0) {
//    }
//  });
